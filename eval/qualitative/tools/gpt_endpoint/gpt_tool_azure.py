...
DEFAULT_PROMPT_REAL_IMAGE  = """
You are an expert in computer vision evaluation, specializing in image restoration and object detection.
Your job is to act as a STRICT and CONCISE judge.

GENERAL RULES
- Judge everything RELATIVE TO THE FOGGY INPUT (F). Do NOT reward 'no change'.
- Only award high scores (4–5) when the Candidate (C) is clearly much better than F, as if most haze were removed with keeping the same consistency of the image.
...
- if the image has many dark spots or is very dark, give a low score (0–2)-> dark does not mean removale of haze

You are evaluating a DEHAZED Candidate (C) against its FOGGY input (F).
Look at both carefully and score each category using the anchors below.
...
1) Visibility Restoration (0–5)
   ...
   - 5 = strong, natural-looking restoration across depths (as if haze mostly removed)

2) Boundary Clarity (0–5)
   Sharpness and definition of contours and thin structures in C vs F.
   ...
   - 5 = crisp, well-aligned boundaries throughout

3) Perceived Detectability (0–5)
   How much easier generic object detection would be on C vs F.
   - 0 = harder than F
   - 1–2 = slight gains; many objects still ambiguous
   - 3 = large/near objects detectable; small/far objects unclear
   - 4 = most objects clearly detectable
   - 5 = very good detectability across the scene, including smaller/far objects
...
4) Visual Distortion (0–5)
   ...
   - 4 = minor artifacts
   - 5 = no visible artifacts; natural-looking image

5) Scene Consistency (0–5)
   Preservation of global layout and geometry in C relative to F.
   ...
   - 5 = layout and geometry closely match F
...
6) Object Consistency (0–5) — PRESENCE/PLACEMENT ONLY (no counting)
   Agreement between C and F in where major object types (cars, people, signs/lights, buses/trucks) appear.
   ...
   - 5 = object presence/placement closely matches F
   * Do NOT use numeric counts. Judge coarse presence/placement across the scene.

7) Relation Consistency (0–5, diagnostic; NOT used in total)
   Do key spatial relations between objects in C match those in F? (e.g., car on road; sign above road; building behind car)
   - 0 = pervasive mismatches
   - 1–2 = several clearly wrong relations
   - 3 = mostly correct; some relation errors
   - 4 = relations almost all correct; minor issues
   - 5 = relations closely match F

   Evidence rule:
     • If relation_consistency ≥ 4, include ≥1 concrete confirmation (e.g., "sign above road in center matches F").
     • If relation_consistency ≤ 2, include ≥1 concrete mismatch in relation_mismatches.
   Return:
     "relation_consistency": int,
     "relation_mismatches": [
       {"subject":"car","relation":"on","object":"road","region":"MC","mismatch":"missing|spurious|misplaced"}
     ]
...
- Be strict: default low unless C is clearly much better than F.
- Explanation: 2–4 short sentences comparing C to F; mention concrete evidence (e.g., “far-left building edges remain soft,” “traffic sign in top-right is now legible,” “sky exhibits haloing around poles”).
- Compute `total` as:
  ...
  "relation_mismatches": [
    {
      ...
      "object": "string",
      "region": "string",
      "mismatch": "missing|spurious|misplaced"
    }
  ],
  "explanation": "string"
}
...
class AzureVLMScorer:
    ...
    def score_double(
        ...
        prompt = prompt_override or DEFAULT_PROMPT_REAL_IMAGE

        foggy_b64 = _b64_data_uri(gt_path)
        cand_b64  = _b64_data_uri(cand_path)
